...
getconfuMat <- function(testdata, rfmodel){
  ...
  jBluesFun <- colorRampPalette(brewer.pal(n = 6, "Blues"))
  paletteSize <- 256
  ...
  ggplotConfusionMatrix <- function(m){
    p <-
      ...
      scale_fill_gradient2(
        ...
        midpoint = (max(data.frame(m$table)$Freq) + min(data.frame(m$table)$Freq)) / 2,
        name = "") +
      ...
      theme(legend.key.height = unit(2.4, "cm"),plot.title = element_text(hjust = 0.5))+
